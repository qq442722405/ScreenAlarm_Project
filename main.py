            ...
            while self.running:
                ...
                for box in list(self.boxes):
                    ...
                    try:
                        ...
                        # 构建多重变体图像 (3通道 NumPy 数组供 PaddleOCR 使用)
                        attempts = []
                        attempts.append(scaled_bgr)

                        gray = cv2.cvtColor(scaled_bgr, cv2.COLOR_BGR2GRAY)
                        attempts.append(cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR))

                        inverted = cv2.bitwise_not(gray)
                        attempts.append(cv2.cvtColor(inverted, cv2.COLOR_GRAY2BGR))
