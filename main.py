...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .trend-slot { display: inline-flex; width: 26px; min-width: 26px; height: 20px; align-items: center; justify-content: center; margin-right: 4px; }
        ...
        .aligned-val { display:inline-flex; align-items:baseline; font-variant-numeric: tabular-nums; }
        .aligned-val .val-int { display:inline-block; min-width:4ch; text-align:right; }
        .aligned-val .val-dot { display:inline-block; width:1ch; text-align:center; }
        .aligned-val .val-frac { display:inline-block; min-width:4ch; text-align:left; }
...
<body>
            ...
            <div id="header-top-panel">
                <div class="header-row1">
                    ...
                    <div class="header-tools-box">
                        <button id="btn-pc-mute" class="btn-sound" onclick="togglePcMute()">🔊 PC声音</button>
    ...
    <script>
        ...
        function updateAuthUI() {
            ...
            document.querySelectorAll('.auth-settings').forEach(el => {
        ...
        async function togglePcMute() {
            await postAction('toggle_pc_mute', -1);
        }
            ...
            boxes.forEach(box => {
                ...
                if (!header) {
                    ...
                    card.innerHTML = `
                        <div class="card-header">
                            <div class="card-title-box">
                                <span id="toggle-icon-${box.id}" class="toggle-icon" style="display: inline-flex;" onclick="toggleSingleCard(${box.id})">${isExpanded ? '▲' : '▼'}</span>
                            ...
                            <div class="card-header-right">
                                <div class="val-container">
                                    ...
                                    <span id="val-${box.id}" class="${valClass}">${alignedValueHtml(box.val_text)}</span>
                                </div>
                        ...
                        <div id="fold-body-${box.id}" class="fold-body" style="display: ${isExpanded ? 'block' : 'none'};">
                            ...
                            <div class="setting-row auth-settings" style="display: ${currentUser ? 'flex' : 'none'};">
                                <label>下限:</label>
                    ...
                    if (valEl) {
                        ...
                        valEl.innerHTML = alignedValueHtml(box.val_text);
                    }
                ...
                if (isExpanded) drawValueChart(box.id, box.chart_data || []);
            });
        ...
        async function fetchStatus() {
            try {
                ...
                const pcMuteBtn = document.getElementById('btn-pc-mute');
                if (pcMuteBtn) {
                    pcMuteBtn.innerText = data.pc_alarm_muted ? '🔇 PC静音' : '🔊 PC声音';
                    pcMuteBtn.style.background = data.pc_alarm_muted ? '#e65100' : '';
                }
...
class WebServerThread(QThread):
    ...
    def _setup_routes(self):
        ...
        def handle_action():
            ...
            if action == 'toggle_pc_mute':
                self.main_win.pc_alarm_muted = not getattr(self.main_win, 'pc_alarm_muted', False)
                if self.main_win.pc_alarm_muted:
                    self.main_win.alarm_player.stop()
                return jsonify({'status': 'ok', 'pc_alarm_muted': self.main_win.pc_alarm_muted})
    ...
    def _on_value_updated(self, box, time_str, val, raw_text):
        ...
        if is_alarm:
            if not box.user_cleared_alarm:
                ...
                if not box.is_muted and not self.pc_alarm_muted:
        ...
        if not any_alarm or self.pc_alarm_muted:
            self.alarm_player.stop()
