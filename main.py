...
class OverlayRegionWidget(QWidget):
    ...
    def __init__(self, box_id, x, y, w, h, name="区域", lower=0.0, mid_op=">", mid_val=50.0, upper=100.0, decimal_places=0, parent=None):
        ...
        self.name = name
        self.lower = lower
        self.mid_op = mid_op if mid_op in [">", "<", "="] else ">"  # 预警逻辑判断符 (默认 >)
        ...
        self.combo_mid_op.setFixedSize(36, 20)
        self.combo_mid_op.setStyleSheet("background-color: rgba(26, 26, 38, 0.5); color: #ffaa00; border: 1px solid #ffaa00; font-size: 10px; border-radius: 2px;")
    ...
    def _on_mid_op_changed(self, op):
        self.mid_op = op
        self.config_changed.emit()
    ...
    def update_result_display(self, val, raw_text=""):
        if val is not None:
            ...
            if self.mid_op == ">":
                ...
                is_warn = (abs(val - self.mid_val) < 1e-5)
    ...
    def paintEvent(self, event):
        ...
        elif self.is_warning:  # 超过预警值变成黄色
        ...
        function renderCardDOM(cardEl, b, isCollapsed, isWarning, currentTimeStr) {
                    ...
                    cardEl.innerHTML = `
                        ...
                        <div class="fold-body">
                            <div class="setting-row" id="setting-row-${b.id}">
                                ...
                                <label>预警:</label>
                                <select id="select-mid-op-${b.id}" class="setting-input" style="width: auto; padding: 2px;">
                                    <option value=">" ${b.mid_op === '>' ? 'selected' : ''}>&gt;</option>
                                    <option value="<" ${b.mid_op === '<' ? 'selected' : ''}>&lt;</option>
                                    <option value="=" ${b.mid_op === '=' ? 'selected' : ''}>=</option>
                                </select>
            ...
            if (isGridView) {
                ...
                if (isLoggedIn) {
                    ...
                    if (midOpSelect && document.activeElement !== midOpSelect) midOpSelect.value = b.mid_op || '>';
...
class GlobalControlPanel(QWidget):
    def __init__(self):
        ...
        self.btn_web_service.clicked.connect(self._open_web_dialog)
        row2_extra_layout.addWidget(self.btn_web_service)
        ...
        self.btn_toggle_edit = QPushButton("✏ 编辑模式")
        self.btn_toggle_edit.setFixedHeight(26)
        self.btn_toggle_edit.clicked.connect(self._toggle_edit)
        row2_extra_layout.addWidget(self.btn_toggle_edit)

        self.btn_collapse = QPushButton("📁 收起")
        self.btn_collapse.setFixedHeight(26)
        self.btn_collapse.clicked.connect(self._toggle_panel_collapse)
        row2_extra_layout.addWidget(self.btn_collapse)
        ...
        # 需求二：网页服务默认自动启动
        self.start_web_service_with_ip(get_local_ip())
    ...
    def stop_web_service(self):
        if self.web_thread and self.web_thread.isRunning():
            ...
            self.web_thread.wait(500)
            self.web_thread = None

    def handle_web_action(self, action, box_id, payload):
        if action == 'toggle_monitor':
            ...
            if box:
                if 'lower' in payload:
                    box.spin_lower.setValue(float(payload['lower']))
                if 'mid_op' in payload:
                    box.combo_mid_op.setCurrentText(str(payload['mid_op']))
                if 'mid_val' in payload:
                    box.spin_mid.setValue(float(payload['mid_val']))
                if 'upper' in payload:
                    box.spin_upper.setValue(float(payload['upper']))

    def _get_box_by_id(self, box_id):
        for box in self.boxes:
    ...
    def _start_add_box(self):
        ...
        time.sleep(0.2)
        self.picker = CoordinatePicker()
        self.picker.coord_selected.connect(self._on_box_selected)
        self.picker.showFullScreen()

    def _on_box_selected(self, x, y, w, h):
        self.show()
        if w > 0 and h > 0:
            box_id = int(time.time() * 1000) % 1000000
            box_name = f"选框_{len(self.boxes)+1}"
    ...
    def _toggle_edit(self):
        ...
        btn_txt = "✅ 完成编辑" if self.is_editing else "✏ 编辑模式"
        btn_style = "background-color: #e65100; color: white; font-weight: bold;" if self.is_editing else "background-color: rgba(43, 45, 66, 0.6); color: white;"
        self.btn_toggle_edit.setText(btn_txt)
        self.btn_toggle_edit.setStyleSheet(btn_style)
    ...
    def _toggle_monitor(self):
        if not self.monitoring:
            ...
            self.btn_monitor.setStyleSheet("background-color: rgba(43, 45, 66, 0.6); color: white;")
    ...
    def _on_monitor_cd(self, sec):
        self.curr_monitor_cd = sec
    ...
    def _on_grille_cd(self, sec):
        self.curr_grille_cd = sec
    ...
    def _check_global_alarm_sound(self):
        ...
        if any_alarm and self.monitoring:
