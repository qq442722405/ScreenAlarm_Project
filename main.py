...
class ScriptEditorDialog(QDialog):
    def __init__(self, script_data=None, parent=None):
        ...
        self.table.setHorizontalHeaderLabels(["步骤", "步骤类型", "详细参数与操作 (支持手动填写坐标或参数)", "删除"])
    ...
    def _add_step_row(self, step=None):
        ...
        def setup_param_ui(selected_text, current_step=None):
            ...
            if selected_text == "点击拾取":
                ...
                edit_x.setPlaceholderText("手动填X")
                edit_x.setMinimumWidth(70)
                
                lbl_y = QLabel("Y:")
                edit_y = QLineEdit(str(y_val) if y_val >= 0 else "")
                edit_y.setPlaceholderText("手动填Y")
                edit_y.setMinimumWidth(70)

                # 把输入框直接挂到这一行参数容器上，保存时精确读取，
                # 不受“拾取”按钮、布局顺序或界面重绘影响。
                ...
                # 关联点击拾取并自动填入输入框
                def do_pick():
                    # 不再在 Qt 主线程中 sleep，避免拾取后对话框焦点/模态状态异常。
                    # 先保存当前输入框引用，拾取结束后再恢复编辑窗口。
                    self.hide()
                    ...
                    def on_selected(px, py):
                        if px >= 0 and py >= 0:
                            edit_x.setText(str(px))
                            edit_y.setText(str(py))

                        # 等拾取窗口真正关闭后再恢复脚本编辑窗口，
                        # 防止两个窗口同时抢焦点导致“点确定没反应”。
                        QTimer.singleShot(0, self._restore_after_pick)
    ...
    def _on_accept(self):
        ...
        for r in range(self.table.rowCount()):
            ...
            if stype_text == "点击拾取":
                x_val, y_val = -1, -1
                if param_layout:
                    # 优先使用创建这一行时保存的坐标输入框引用。
                    # 不再依赖 QHBoxLayout 中控件的排列顺序，避免拾取后保存时取值失败。
                    try:
                        edit_x_ref = getattr(param_widget, "_script_edit_x", None)
                        edit_y_ref = getattr(param_widget, "_script_edit_y", None)

                        if isinstance(edit_x_ref, QLineEdit) and isinstance(edit_y_ref, QLineEdit):
                            tx = edit_x_ref.text().strip()
                            ty = edit_y_ref.text().strip()
                        else:
                            # 兼容旧行：从布局中查找前两个 QLineEdit
                            line_edits = [
                                param_layout.itemAt(i).widget()
                                for i in range(param_layout.count())
                                if isinstance(param_layout.itemAt(i).widget(), QLineEdit)
                            ]
                            tx = line_edits[0].text().strip() if len(line_edits) >= 1 else ""
                            ty = line_edits[1].text().strip() if len(line_edits) >= 2 else ""
