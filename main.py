...
class OverlayRegionWidget(QWidget):
    ...
    def __init__(self, box_id, x, y, w, h, name="区域", lower=0.0, mid_op=">", mid_val=50.0, upper=100.0, decimal_places=0, parent=None):
        super().__init__(None)
        self.box_id = box_id
        self.capture_x = x
        self.capture_y = y
        ...
        self.name = name
        self.lower = lower
        self.mid_op = mid_op if mid_op in [">", "<", "="] else ">"  # 预警模式（大于、小于、等于，默认大于）
        ...
        self.combo_mid_op.setFixedSize(38, 20)
        self.combo_mid_op.setStyleSheet("background-color: rgba(26, 26, 38, 0.5); color: #ffaa00; border: 1px solid #ffaa00; font-size: 10px; border-radius: 2px; font-weight: bold;")
    ...
    def update_result_display(self, val, raw_text=""):
        if val is not None:
            ...
            is_warn = False
            if self.mid_op == ">":
                is_warn = val > self.mid_val
            elif self.mid_op == "<":
                is_warn = val < self.mid_val
            elif self.mid_op == "=":
                is_warn = abs(val - self.mid_val) < 1e-4
    ...
    def _update_bar_visibility(self):
        if self.panel_hidden:
            ...
            self.combo_mid_op.setEnabled(self.is_editing)
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .setting-select { background: rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.2); border-radius: 4px; color: #ffaa00; font-weight: bold; padding: 3px 2px; width: 38px; font-size: 11px; }
    ...
    <script>
        ...
        function saveLimits(boxId) {
            ...
            if (!isNaN(lowerVal) && !isNaN(midVal) && !isNaN(upperVal)) {
                postAction('set_limits', boxId, { lower: lowerVal, mid_op: midOp, mid_val: midVal, upper: upperVal });
            } else {
                alert("请输入有效的数值！");
            }
        }
        ...
        function renderCardDOM(cardEl, b, isCollapsed, isWarning, currentTimeStr) {
                    ...
                    cardEl.innerHTML = `
                        ...
                        <div class="fold-body">
                            <div class="setting-row" id="setting-row-${b.id}">
                                ...
                                <label>预警:</label>
                                <select id="select-mid-op-${b.id}" class="setting-select">
                                    <option value=">" ${midOp === '>' ? 'selected' : ''}>&gt;</option>
                                    <option value="<" ${midOp === '<' ? 'selected' : ''}>&lt;</option>
                                    <option value="=" ${midOp === '=' ? 'selected' : ''}>=</option>
                                </select>
            ...
            if (isGridView) {
                ...
                if (isLoggedIn) {
                    ...
                    const midOpSelect = document.getElementById(`select-mid-op-${b.id}`);
                    ...
                    if (midOpSelect && document.activeElement !== midOpSelect) midOpSelect.value = midOp;
            ...
            try {
                ...
                sortedBoxes.forEach(b => {
                    ...
                    if (!b.is_alarm && !isNaN(numVal)) {
                        if (op === '>') isWarning = numVal > b.mid_val;
                        else if (op === '<') isWarning = numVal < b.mid_val;
                        else if (op === '=') isWarning = Math.abs(numVal - b.mid_val) < 0.0001;
                    }
...
class GlobalControlPanel(QWidget):
    def __init__(self):
        ...
        self.btn_add_box = QPushButton("➕ 选框")
        ...
        self.btn_web = QPushButton("🌐 网页")
        self.btn_web.setFixedHeight(26)
        self.btn_web.clicked.connect(self._open_web_dialog)
        row2_extra_layout.addWidget(self.btn_web)

        row2_layout.addWidget(self.row2_extra_container)

        self.btn_collapse = QPushButton("🔼 收起")
        self.btn_collapse.setFixedSize(60, 26)
        self.btn_collapse.setStyleSheet("background-color: rgba(0, 136, 204, 0.8); color: white; border-radius: 4px; font-weight: bold;")
        ...
        # Requirement 2: 网页服务默认启动
        QTimer.singleShot(200, lambda: self.start_web_service_with_ip("0.0.0.0"))
    ...
    def _init_ocr(self):
        def load_thread():
            try:
                ...
                print("DDDDOCR 加载失败:", e)
        threading.Thread(target=load_thread, daemon=True).start()

    def start_web_service_with_ip(self, ip_str):
        if self.web_thread and self.web_thread.isRunning():
            self.stop_web_service()
        self.web_thread = WebServerThread(self, host=ip_str, port=5000)
    ...
    def _handle_web_action(self, action, box_id, payload):
        if action == 'toggle_monitor':
            ...
            for b in self.boxes:
                if b.box_id == box_id:
                    b.lower = payload.get('lower', b.lower)
                    b.mid_op = payload.get('mid_op', b.mid_op)
                    b.mid_val = payload.get('mid_val', b.mid_val)
                    b.upper = payload.get('upper', b.upper)
    ...
    def _on_monitor_tick(self, rem_sec):
    ...
    def _toggle_grille(self):
        if self.grille_thread and self.grille_thread.isRunning():
            ...
            self.btn_grille_start.setStyleSheet("background-color: #b03a3a; color: white; font-weight: bold;")
    ...
    def _toggle_edit(self):
        ...
        self.btn_edit.setText("🔒 锁框" if self.is_editing else "✏️ 编辑")
        for b in self.boxes:
    ...
    def add_box(self, x, y, w, h, name=None, lower=0.0, mid_op=">", mid_val=50.0, upper=100.0, decimal_places=0, is_muted=False):
        box_id = len(self.boxes) + 1
        if not name:
            name = f"区域 {box_id}"

        b = OverlayRegionWidget(box_id, x, y, w, h, name=name, lower=lower, mid_op=mid_op, mid_val=mid_val, upper=upper, decimal_places=decimal_places)
        b.is_muted = is_muted
        b.btn_mute.setText("🔇" if is_muted else "🔊")
        ...
        b.config_changed.connect(self.save_config)
    ...
    def _check_global_alarm(self):
        any_active = any(b.is_alarm and not b.is_muted for b in self.boxes)
        if any_active:
        ...
        try:
            ...
            for box_data in data.get('boxes', []):
                self.add_box(
                    ...
                    is_muted=box_data.get('is_muted', False)
                )
        except Exception as e:
            print("加载配置失败:", e)
