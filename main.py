...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.record_interval_spin.setValue(60)       # 默认60分钟
    ...
    def test_selected_point(self):
        ...
        # 懒加载 OCR reader（独立于监控线程）
        if not hasattr(self, 'test_reader') or self.test_reader is None:
            try:
                import easyocr
                self.test_reader = easyocr.Reader(['en'], gpu=False, verbose=False)
            except Exception as e:
                QMessageBox.critical(self, "错误", f"OCR 引擎加载失败: {e}")
                return
